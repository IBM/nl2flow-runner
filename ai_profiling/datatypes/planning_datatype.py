...
class SimplePlanningModel(BaseModel):
    ...
    mappings: Union[List[Tuple[str, str]], List[Tuple[str, str, float]]] = []
    ...
    def get_stat(self, true_model: SimplePlanningModel) -> JsonTranslationStatModel:
        return JsonTranslationStatModel(
            ...
            num_available_data=self.get_stat_lists(
                true_list=true_model.available_data,
                response_list=true_model.available_data,  # momory was not used for this study in 2025
            ),
...
class AgentInfoUnitModel(BaseModel):
    ...
    @staticmethod
    def get_agent_info_unit_model_from_pddl_generator_output(
        pddl_generator_output: PddlGeneratorOutput,
    ) -> AgentInfoUnitModel:
        return AgentInfoUnitModel(
            available_agents=pddl_generator_output.agent_info_generator_output_item.available_agents,
            goal_agent_ids=pddl_generator_output.agent_info_generator_output_item.goal_agent_ids,
            mappings=pddl_generator_output.agent_info_generator_output_item.mappings,
            available_data=pddl_generator_output.agent_info_generator_output_item.available_data,
        )
    ...
    def get_simple_planning_model_json_str(self) -> str:
        ...
        return simple_planning_model.model_dump_json()
